...
describe Router do
  ...
  it 'give the square upon square command' do
    expect(router.map Parser.new("sqr")).to eq(0)
  end

  it 'give the square root upon square root command' do
    expect(router.map Parser.new("sqrt")).to eq(0)
  end

  it 'give the absolute value upon absolute command' do
    expect(router.map Parser.new("abs")).to eq(0)
  end

  it 'give the negative value upon neg command' do
    expect(router.map Parser.new("neg")).to eq(0)
  end

  it 'give the cube value upon cube command' do
    expect(router.map Parser.new("cube")).to eq(0)
  end

  it 'give the cube root value upon cubert command' do
    expect(router.map Parser.new("cubert")).to eq(0)
  end

  it 'give error if input is divide 0' do
    lambda { router.map Parser.new("divide", 0) }.should raise_error
  end
